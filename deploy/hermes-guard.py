#!/usr/bin/env python3
"""
hermes-guard — Hermes Agent integrity watchdog.
Runs as root for iptables access, monitors /home/ubuntu/.hermes
"""
...
HERMES_VENV_BIN = f"{HERMES_HOME}/hermes-agent/venv/bin/python"
HERMES_CONFIG = f"{HERMES_HOME}/config.yaml"
HERMES_ENV = f"{HERMES_HOME}/.env"
AUTH_KEYS = f"{HOME}/.ssh/authorized_keys"
GUARDIAN_DIR = "/opt/tical-guardian"
BASELINE_FILE = f"{GUARDIAN_DIR}/hermes_baseline.json"
EMERGENCY_DIR = f"{GUARDIAN_DIR}/emergency"
PATROL_LOG = f"{GUARDIAN_DIR}/hermes_guard.log"
CHECKSUM_FILE = f"{GUARDIAN_DIR}/.hermes_checksums"
PATROL_INTERVAL = 120
GATEWAY_PORT = 8642
...
def hash_all() -> Dict:
    return {
        "hermes_venv_bin": sha256_file(HERMES_VENV_BIN),
        "hermes_config": sha256_file(HERMES_CONFIG),
        "hermes_env": sha256_file(HERMES_ENV),
        "auth_keys": sha256_file(AUTH_KEYS),
        "hermes_env_perm": oct(os.stat(HERMES_ENV).st_mode)[-3:] if os.path.exists(HERMES_ENV) else "MISSING",
    }
...
def fire_emergency(reason: str, detail: str) -> None:
    alert = {"type":"HERMES_INTEGRITY","reason":reason,"detail":detail,
             "action":"iptables DROP","node":os.uname().nodename,
             "timestamp":datetime.now(timezone.utc).isoformat()}
    fname = f"hermes_{int(time.time())}.json"
    ...
    subprocess.run(["logger","-t","hermes-guard","-p","crit",f"INTEGRITY_BREACH: {reason}"], check=False)
...
def patrol(baseline: Dict) -> bool:
    cur = hash_all()
    violations = []
    if cur["hermes_env_perm"] not in ("600","400","MISSING"):
        violations.append(f".env perm {cur['hermes_env_perm']} (need 600)")
    for key, label in [("hermes_venv_bin","gateway binary"),("hermes_config","config.yaml"),
