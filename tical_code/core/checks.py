...
from tical_code.core.module_wiring_check import check_module_wiring
...
def _get_worker_service() -> str:
    """Detect the active worker systemd service name."""
    ...
    # LIVE 2026-07-09p2: dynamic service discovery via systemctl
    import subprocess as _sp2
    try:
        _r = _sp2.run(["systemctl", "list-units", "--type=service", "--state=active", "--no-legend"],
...
def check_worker_alive() -> Tuple[bool, str]:
    """Check if the worker process is running (systemd or direct)."""
    import subprocess as _sp
    ...
    if svc:
        try:
            ...
            pass
    # Fallback: check for running unified_worker process
    try:
        r = _sp.run(["pgrep", "-f", "unified_worker"],
                   capture_output=True, text=True, timeout=5)
        if r.stdout.strip():
            pids = r.stdout.strip().split("\n")
            return True, f"Worker running directly (PID: {pids[0]})"
    except Exception:
        pass
    return False, "No active worker found"
...
def check_ssh_access() -> Tuple[bool, str]:
    """Verify ~/.ssh/authorized_keys exists with valid keys and correct perms."""
    ssh_dir = Path.home() / ".ssh"
    ak_file = ssh_dir / "authorized_keys"

    # Check directory exists
    if not ssh_dir.exists():
        return False, "~/.ssh/ directory missing"

    # Check authorized_keys exists
    if not ak_file.exists():
        return False, "authorized_keys file missing"

    # Check file is not empty and contains valid SSH key lines
    try:
        content = ak_file.read_text(encoding="utf-8").strip()
    except Exception:
        return False, "authorized_keys unreadable"

    if not content:
        return False, "authorized_keys is empty"

    # Verify at least one valid SSH key line
    key_pattern = re.compile(r'^(ssh-(?:rsa|ed25519|dss|ecdsa)|ecdsa-sha2-nistp\d+) ')
    has_valid_key = any(key_pattern.match(line) for line in content.splitlines())
    if not has_valid_key:
        return False, "no valid SSH key found in authorized_keys"
    key_count = sum(1 for line in content.splitlines() if key_pattern.match(line))

    # Check permissions
    try:
        mode = ak_file.stat().st_mode & 0o777
        if mode != 0o600:
            return False, f"authorized_keys has wrong permissions: {oct(mode)} (expected 0o600)"
    except Exception:
        return False, "cannot stat authorized_keys"

    return True, f"{key_count} valid SSH key(s), permissions ok (0o600)"


def check_task_queue() -> Tuple[bool, str]:
    """Check for stuck or old task files."""
    tasks_dir = under_tical_home("tasks")
...
_CHECK_REGISTRY: List[Tuple[callable, str, str]] = [
    ...
    (check_ssh_access,        "ssh_access",       "P0"),
    # ── Runtime wiring verification ──
    (check_module_wiring,     "module_wiring",    "P0"),
]
