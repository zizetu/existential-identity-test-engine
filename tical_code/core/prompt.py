...
def build_system_prompt(
    ...
    rules = [
        ...
        "- **MANDATORY COMPLETION SIGNAL**: Every task MUST end by calling `end_task` or `task_complete`. Without this signal the task loop will NOT stop. Do NOT assume the loop will auto-terminate.",
        "- **Knowledge questions**: Answer in text + call `end_task` with success=true, OR include `[TASK_COMPLETE]` at the end.",
        "- **Code/action tasks**: After making changes, call `end_task` with success=true/false. NEVER just reply with text and stop.",
        ...
        "- **Search limit**: For research tasks, limit web_fetch/search to 5 calls. After gathering enough info, synthesize and call `end_task`.",
